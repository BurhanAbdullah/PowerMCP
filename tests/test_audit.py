...
def test_nan_and_infinite_values_are_detected():
    ...
    assert {"BUS_MIN_VM_INVALID", "BUS_MAX_VM_INVALID", "LINE_INVALID_REACTANCE"} <= codes
...
def test_invalid_line_and_transformer_parameters_are_reported():
    ...
    report = audit_network(net)
    codes = {f.code for f in report.findings}
    assert {"LINE_INVALID_LENGTH", "LINE_INVALID_RESISTANCE", "LINE_ZERO_IMPEDANCE", "LINE_INVALID_RATING", "TRAFO_INVALID_RATING", "TRAFO_INVALID_SHORT_CIRCUIT"} <= codes
